...
const AnnualReportsContent = () => {
  ...
  const [reports, setReports] = useState<AnnualReport[]>(mockReports);
  const [selectedReports, setSelectedReports] = useState<string[]>([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(5);
  const [sortConfig, setSortConfig] = useState<{
    key: keyof AnnualReport | null;
    direction: "asc" | "desc";
  ...
  const handleSort = (key: keyof AnnualReport) => {
    setSortConfig((prev) => ({
      key,
        ...
        <TableBody>
          {paginatedReports.map((report, index) => (
            ...
            >
              ...
              <TableCell className="font-medium w-40">{report.reportName}</TableCell>
