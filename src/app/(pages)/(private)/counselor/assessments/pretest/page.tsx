...
function PretestRow({ therapy }: { therapy: any }) {
  ...
  const { data: pretest, isLoading: pretestLoading } = usePretestByTherapy(
    therapy._id
  );
        ...
        {pretestLoading ? (
          ...
          <div className="flex items-center gap-2">
            {hasPretest ? (
              ...
              <Link
                href={`/counselor/assessments/pretest/create?therapyId=${therapy._id}`}
...
export default function PretestAssessmentsPage() {
  ...
  return (
    <div className="">
      {/* Header */}
      <div className="mb-6">
        <Heading3 className="text-neutral-02">Asesmen Pretest (SDQ)</Heading3>
        <p className="text-grey mt-2">Asesmen awal sebelum terapi dimulai</p>
      </div>
            ...
            <tbody className="divide-y divide-grey-stroke">
              {isLoading ? (
                ...
                therapies.map((therapy) => (
                  <PretestRow key={therapy._id} therapy={therapy} />
                ))
              ) : (
                <tr>
