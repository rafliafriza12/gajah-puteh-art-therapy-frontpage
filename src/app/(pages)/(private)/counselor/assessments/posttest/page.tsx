...
function PosttestRow({ therapy }: { therapy: any }) {
  ...
  const { data: posttest, isLoading: posttestLoading } = usePosttestByTherapy(
    therapy._id
  );
        ...
        {posttestLoading ? (
          ...
          <div className="flex items-center gap-2">
            {hasPosttest ? (
              ...
              <Link
                href={`/counselor/assessments/posttest/create?therapyId=${therapy._id}`}
...
export default function PosttestAssessmentsPage() {
  ...
  return (
    <div className="">
      {/* Header */}
      <div className="mb-6">
        <Heading3 className="text-neutral-02">Asesmen Posttest (SDQ)</Heading3>
        <p className="text-grey mt-2">Asesmen akhir setelah terapi selesai</p>
      </div>
            ...
            <tbody className="divide-y divide-grey-stroke">
              {isLoading ? (
                ...
                therapies.map((therapy) => (
                  <PosttestRow key={therapy._id} therapy={therapy} />
                ))
              ) : (
                <tr>
